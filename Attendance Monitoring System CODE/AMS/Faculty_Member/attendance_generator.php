...
$sql = "SELECT * FROM schedule WHERE 
        schedule_day = '$currentDayOfWeek' AND 
        TIME_FORMAT(schedule_time_start, '%H:%i') <= TIME_FORMAT('$currentTime', '%H:%i') AND 
        (cooldown > '$currentTime' OR cooldown IS NULL) AND 
        request IS NULL"; // Added condition for the 'request' column
...
$sql = "SELECT * FROM request WHERE schedule_date <= '$currentDate' AND generate = 'active' AND TIME_FORMAT(schedule_time_start, '%H:%i') <= TIME_FORMAT('$currentTime', '%H:%i') AND status = 'approved'";
...
// Script 4: Insert rows from request table to schedule table
$insertSql = "INSERT INTO schedule (user_id, subject_code, subject_units, schedule_time_start, schedule_time_end, room, schedule_day, request, section, alert, schedule_date) 
              SELECT 
                  IF(request.sub_professor IS NULL, request.user_id, 
                     (SELECT user_id FROM user WHERE user_fullname = request.sub_professor)) AS user_id,
                  request.subject_code, request.subject_units, request.schedule_time_start, 
                  request.schedule_time_end, request.room, 'Today' AS schedule_day, 'True' AS request, request.section, 'True' AS alert,
                  request.schedule_date  -- Added the schedule_date column
              FROM request
              WHERE schedule_generate = '1' AND status = 'approved'";

if ($conn->query($insertSql) === TRUE) {
    // Update schedule_generate to NULL for the inserted rows
    $updateSql = "UPDATE request SET schedule_generate = NULL 
                  WHERE schedule_generate = '1' AND status = 'approved'";

    if ($conn->query($updateSql) === TRUE) {
        //echo "New rows added to schedule table from request table, schedule_generate updated, and alert set to 'True' successfully.\n";
    } else {
        //echo "Error updating schedule_generate: " . $conn->error . "\n";
    }
} else {
...
// Script 5: Delete rows from schedule table
$deleteSql = "DELETE FROM schedule 
              WHERE request = 'True' AND 
                    TIME_FORMAT(schedule_time_end, '%H:%i') < TIME_FORMAT('$currentTime', '%H:%i')";

if ($conn->query($deleteSql) === TRUE) {
    //echo "Rows deleted from schedule table successfully.\n";
} else {
