...
function downloadTable() {
  ...
  const sanitizedUserName = sanitizeFileName(userName.trim());
  const selectedMonth = document.getElementById("status").value;
  const monthName = selectedMonth === "current_month" ? new Date().toLocaleString('default', { month: 'long' }) : selectedMonth;
  const fileName = sanitizedUserName + '_' + monthName + '_table_data.csv'; // Include the user's name and selected month in the file name
...
// Sanitize the file name by removing special characters and limiting the length
function sanitizeFileName(fileName) {
  const maxLength = 30; // Maximum length for the file name
  const sanitizedFileName = fileName.replace(/[^a-zA-Z0-9]/g, ''); // Remove special characters
